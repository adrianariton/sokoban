...
class Map:
    ...
    def __init__(
        self, length, width, player_x, player_y, boxes, targets, obstacles, test_name="test"
    ):
        self.length = length
        self.width = width
    ...
    def set_from_signature(self, sig):
        """
        ...
        """
        # Extract player position and box positions from signature
        player_x, player_y, box_positions = sig

        # Update player position
        self.player.x = player_x
        self.player.y = player_y

        # Clear existing boxes from the map
        for box_x, box_y in list(self.positions_of_boxes.keys()):
            self.map[box_x][box_y] = 0

        # Reset positions of boxes dictionary
        self.positions_of_boxes = {}
    ...
    def signature(self) -> tuple:
        ...
        """
        # get a sorted tuple of all box coords
        boxes = tuple(sorted(self.positions_of_boxes.keys()))
        return (self.player.x, self.player.y, boxes)
    ...
    def box_valid_move(self, move):
        ...
        if self.player_valid_move(implicit_move):
            ...
            if self.map[future_position[0]][future_position[1]] == BOX_SYMBOL:
                straight_move_flag = self.object_valid_move(
                    self.boxes[self.positions_of_boxes[future_position]], implicit_move
                )
            ...
            if 0 <= opposite_position[0] < self.length and 0 <= opposite_position[1] < self.width:
                if self.map[opposite_position[0]][opposite_position[1]] == BOX_SYMBOL:
                    straight_move_flag = straight_move_flag or self.object_valid_move(
                        self.boxes[self.positions_of_boxes[opposite_position]], implicit_move
                    )
    ...
    def copy(self):
        ...
        new_map = Map(
            ...
            self.player.x,
            self.player.y,
            [(box.name, box.x, box.y) for box in self.boxes.values()],
            self.targets,
            self.obstacles,
        )
    ...
    def _create_figure(
        self, show: bool = True, save_path: Optional[str] = None, save_name: Optional[str] = None
    ) -> None:
        fig, ax = plt.subplots()
